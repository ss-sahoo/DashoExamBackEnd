...
def list_centers(request):
    ...
    # Filter by role
    if user.role == User.ROLE_ADMIN and user.center:
...
def get_center(request, center_id: str):
    ...
    # Check permissions
    user = request.user
    if user.role == User.ROLE_ADMIN and user.center and user.center.id != center.id:
        return Response(
...
def list_center_users(request, center_id: str):
    ...
    for user_obj in users:
        ...
        if user_obj.role == User.ROLE_TEACHER:
            user_data.update({
        ...
        elif user_obj.role == User.ROLE_STUDENT:
            user_data.update({
